...
def handle_gemini_request() : # ✅ เปลี่ยนชื่อฟังก์ชันเพื่อป้องกันความสับสน
    ...
    # 2. ✅ แก้ไข: ต้องใช้ request.files.get() เพื่อดึงไฟล์รูปภาพ
    image_file = request.files.get('imageFile') 

    # 3. ตรวจสอบและ Debug
    print(f"Received Prompt: '{user_prompt}'")
    # 💡 ใช้ image_file (ตัวพิมพ์เล็ก)
    print(f"Received Image Filename: {image_file.filename if image_file else 'None'}") 
    ...
    if image_file:
        try:
            # 💡 ใช้ PILImage (ชื่อคลาสใหม่)
            img = PILImage.open(image_file.stream) 
            contents.append(img)
        except Exception as e:
            # ถ้าโหลดไฟล์ไม่ได้ จะส่ง 400 กลับไป
            return jsonify({'error': f'Cannot process image file (PIL Error): {str(e)}'}), 400
